...
class _WidgetCallSiteTransformer extends Transformer {
  ...
  ///判断是否是项目的采集路径
  bool _isProject(Location location) {
    if (_rootUrlList != null && _rootUrlList!.isNotEmpty) {
      for (String url in _rootUrlList!) {
        if (location.file.toString().startsWith(url)) {
          return true;
        }
      }
    }
