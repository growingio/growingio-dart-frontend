...
if [ $# -ne 2 ]; then
    echo "[flutter_snapshot_dump] 请输入正确的flutter版本号和Dart版本号."
    exit 1
fi
...
echo "[flutter_snapshot_dump] start checkout flutter tag with $flutter_version"
cd "$FLUTTER_SOURCE_DIR"
git pull
git checkout -f "$flutter_version"

echo "[flutter_snapshot_dump] start checkout dart tag with $dart_version"
cd "$DART_SOURCE_DIR"
git pull
